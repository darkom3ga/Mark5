...
start_command=" cd ~/h264final_fixed/Sender ; python3 hevc_indexfix4.py "
...
class SSHManager:
    ...
    def disconnect(self):
        if self.is_ssh_connected:
            try:
                ssh_stdin, ssh_stdout, ssh_stderr = self.ssh.exec_command("kill -9 $(pgrep -f 'python3 hevc_indexfix4.py')")
                output = ssh_stdout.readlines()
                print(output)
                self.ssh.exec_command("pkill -f 'python3 hevc_indexfix4.py'")  
                self.ssh.close()
